const VidstackHook = {
    ...
    getFrameDuration() {
      // Try to get actual framerate if available
      // Most browsers don't expose fps directly, so we use a default if not available
      const fps = this.videoElement.getVideoPlaybackQuality?.()?.totalVideoFrames / this.videoElement.duration || 30;
      return 1 / fps;
    },
