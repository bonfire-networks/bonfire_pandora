const VidstackHook = {
    mounted() {
        ...
        this.seekFromParamsIfPresent();
      },
    

      setupTimestampBadgeClickHandlers() {
        // Use event delegation so clicks work on dynamically added elements (e.g. thread annotations)
        ...
        this._annotationClickHandler = handleClick;
      },

      seekFromParamsIfPresent() {
        const seekIn = parseFloat(this.el.dataset.seekIn);
        if (!isNaN(seekIn) && this.videoElement) {
          const doSeek = () => {
            this.videoElement.currentTime = seekIn;
            this.videoElement.play().catch(() => {});
            this.videoElement.removeEventListener('loadedmetadata', doSeek);
          };
          if (this.videoElement.readyState >= 1) {
            doSeek();
          } else {
            this.videoElement.addEventListener('loadedmetadata', doSeek);
          }
        }
      },
      ...
      destroyed() {
        if (this._annotationClickHandler) {
          document.removeEventListener('click', this._annotationClickHandler, true);
        }
      },
