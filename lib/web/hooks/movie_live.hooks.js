...
const PandoraMoviePlayer = {
  ...
  setupPlayerControls() {
    ...
    const volumeSlider = el.querySelector('[data-role="volume-slider"]');
    if (muteBtn || volumeSlider) {
      ...
      const syncVolume = () => {
        const vol = Number.isFinite(this.player.volume) ? this.player.volume : 1;
        if (volumeSlider) volumeSlider.value = String(vol);
        const muted = this.player.muted || vol === 0;
      ...
      if (volumeSlider) {
        volumeSlider.addEventListener("input", () => {
          ...
          if (vol > 0) this.player.muted = false;
        });
      }

      this.player.on("volumechange", syncVolume);
      syncVolume();
    }
