...
/** Letterbox size matching CSS background-size: contain. */
function fitMediaInMonitor(mediaWidth, mediaHeight, boxWidth, boxHeight) {
  if (!mediaWidth || !mediaHeight || !boxWidth || !boxHeight) return null;
...
function syncPandoraPreviewLayout(player) {
  const wrapper = player.elements?.wrapper;
  const media = player.media;
  if (!wrapper || !media) return;

  wrapper.classList.remove("plyr__video-wrapper--fixed-ratio");
  ...
  const inFullscreen = player.fullscreen?.active;

  if (inFullscreen) {
    // Feed letterbox px sizes must not persist in fullscreen — fill the FS container.
...
/** Keep feed preview aligned with poster after Plyr/CSS global rules run. */
function attachPandoraPreviewLayout(player) {
  const wrapper = player.elements?.wrapper;
  if (!wrapper) return () => {};

  const sync = () => requestAnimationFrame(() => syncPandoraPreviewLayout(player));

  player.on("loadedmetadata", sync);
  player.on("loadeddata", sync);
  player.on("play", sync);
  player.on("enterfullscreen", sync);
  player.on("exitfullscreen", sync);
  ...
  return () => {
    ...
    player.off("loadeddata", sync);
    player.off("play", sync);
    player.off("enterfullscreen", sync);
    player.off("exitfullscreen", sync);
    ro.disconnect();
  };
}
...
function createPlyrOnVideo(video, hook) {
  try {
    ...
    // Legacy markup used width/height="320/180" (16:9) — distorts 4:3 when CSS sizes from attrs.
    if (isPandoraPreview) {
    ...
    let cleanup = null;
    if (isPandoraPreview) {
      const cleanups = [attachPandoraPreviewLayout(player)];

      const bounds = parseSegmentBounds(video);
      if (bounds) {
        cleanups.push(attachPandoraSegmentLoop(player, bounds.tIn, bounds.tOut));
      }
...
const PlyrInit = {
  mounted() {
    ...
    // Clicks on PanDoRa previews bubble to the activity <article> (PreviewActivity),
    // which opens the discussion modal. Stop propagation so play/pause stays local.
    this.blockFeedPreviewBubble = (e) => {
      if (e.target.closest(".pandora-video-preview-wrapper")) {
        e.stopPropagation();
      }
    };
    this.el.addEventListener("click", this.blockFeedPreviewBubble);
  },

  destroyed() {
    if (this.blockFeedPreviewBubble) {
      this.el.removeEventListener("click", this.blockFeedPreviewBubble);
    }
