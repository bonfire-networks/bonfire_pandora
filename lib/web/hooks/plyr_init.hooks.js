 ...
 * sul video. A fine segmento icona **loop** sul play in basso a sinistra; nuovo play da `in`.
 */
...
const STYLE_ID = "pandora-plyr-preview-segment-styles-v2";
...
function injectPandoraPlyrPreviewStylesOnce() {
  ...
  el.id = STYLE_ID;
  el.textContent = `
    .${LOOP_UI_CLASS} .plyr__controls button[data-plyr="play"] svg {
      display: none !important;
    }
    .${LOOP_UI_CLASS} .plyr__controls button[data-plyr="play"] {
      position: relative;
      display: inline-flex;
      align-items: center;
      justify-content: center;
    }
    .${LOOP_UI_CLASS} .plyr__controls button[data-plyr="play"]::after {
      content: "↻";
      font-size: 1.2rem;
      line-height: 1;
      color: inherit;
