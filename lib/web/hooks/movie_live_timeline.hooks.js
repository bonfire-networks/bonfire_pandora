...
const PREVIEW_GAP = 8;
const PREVIEW_PAD = 4;
/** Fallback when the tooltip is still `display:none` and has no layout box yet. */
const PREVIEW_W_FALLBACK = 168;
const PREVIEW_H_FALLBACK = 196;

/**
 * Keep the frame preview inside the visible expanded timeline viewport (fixed coords).
 * Prefers above the hovered row; flips below when there is not enough headroom.
 */
function positionFramePreview(previewEl, containerEl, clientX, rowRect) {
  const previewW = previewEl.offsetWidth || PREVIEW_W_FALLBACK;
  const previewH = previewEl.offsetHeight || PREVIEW_H_FALLBACK;
  const cRect = containerEl.getBoundingClientRect();

  const bounds = {
    left: cRect.left + PREVIEW_PAD,
    top: cRect.top + PREVIEW_PAD,
    right: cRect.right - PREVIEW_PAD,
    bottom: cRect.bottom - PREVIEW_PAD,
  };

  let left = clientX - previewW / 2;
  left = clamp(left, bounds.left, Math.max(bounds.left, bounds.right - previewW));

  const rowTop = rowRect.top;
  const rowBottom = rowRect.bottom;
  const spaceAbove = rowTop - bounds.top;
  const spaceBelow = bounds.bottom - rowBottom;
  let top;

  if (spaceAbove >= previewH + PREVIEW_GAP) {
    top = rowTop - previewH - PREVIEW_GAP;
  } else if (spaceBelow >= previewH + PREVIEW_GAP) {
    top = rowBottom + PREVIEW_GAP;
  } else if (spaceAbove >= spaceBelow) {
    top = clamp(rowTop - previewH - PREVIEW_GAP, bounds.top, bounds.bottom - previewH);
  } else {
    top = clamp(rowBottom + PREVIEW_GAP, bounds.top, bounds.bottom - previewH);
  }

  top = clamp(top, bounds.top, Math.max(bounds.top, bounds.bottom - previewH));

  previewEl.style.left = `${left}px`;
  previewEl.style.top = `${top}px`;
}

function pointerInsideExpanded(containerEl, clientX, clientY) {
  if (!containerEl || containerEl.classList.contains("hidden")) return false;
...
const PandoraTimelineStrip = {
  mounted() {
    ...
    this.tileUrlTemplateSmall = this.el.dataset.tileUrlTemplateSmall || "";
    this.tileUrlTemplateLarge = this.el.dataset.tileUrlTemplateLarge || "";
    ...
    this._previewPointerX      = null;
    this._previewPointerY      = null;
    this._onFramePreviewScroll = null;
    ...
    console.debug("[timeline] mounted", {
      tileUrl64: this.tileUrlTemplateLarge ? "ok" : "EMPTY",
      tileUrl16: this.tileUrlTemplateSmall ? "ok" : "EMPTY",
  ...
  updated() {
    ...
    this.tileUrlTemplateSmall = this.el.dataset.tileUrlTemplateSmall || this.tileUrlTemplateSmall;
    this.tileUrlTemplateLarge = this.el.dataset.tileUrlTemplateLarge || this.tileUrlTemplateLarge;
  ...
  destroyed() {
    ...
    if (this._onFramePreviewScroll && this.expandedEl) {
      this.expandedEl.removeEventListener("scroll", this._onFramePreviewScroll);
      this._onFramePreviewScroll = null;
    }
  ...
  rebuildCompactTiles() {
    ...
    const duration = this.effectiveDuration();
    if (!duration || !this.tileUrlTemplateSmall) return;
    this._tilesBuilt.compact = true;

    const totalTiles = Math.ceil(duration / SECONDS_PER_TILE);
    const parts = [];
    for (let i = 0; i < totalTiles; i++) {
      const url = this.tileUrlTemplateSmall.replace("{i}", String(i));
  ...
  buildLargeTimeline() {
    ...
    console.debug("[timeline] buildLargeTimeline", {
      duration,
      tileUrl64: this.tileUrlTemplateLarge ? this.tileUrlTemplateLarge.slice(0, 60) + "…" : "EMPTY",
      expandedEl: !!this.expandedEl,
      alreadyBuilt: this._tilesBuilt.expanded,
    });
    if (!duration || !this.tileUrlTemplateLarge || !this.expandedEl) {
    ...
    for (let i = 0; i < totalTiles; i++) {
      ...
      const url          = this.tileUrlTemplateLarge.replace("{i}", String(i));
  ...
  hideFramePreview() {
    if (this.framePreviewEl) {
      ...
      this.framePreviewEl.setAttribute("hidden", "");
      this.framePreviewEl.setAttribute("aria-hidden", "true");
    }
    this._previewPointerX = null;
    this._previewPointerY = null;
    if (this._onWindowPointerMove) {
      document.removeEventListener("pointermove", this._onWindowPointerMove, true);
      this._onWindowPointerMove = null;
    }
  },

  attachWindowPointerTracking() {
    if (this._onWindowPointerMove) return;

    this._onWindowPointerMove = (event) => {
      if (this.framePreviewEl?.classList.contains("hidden")) return;
      if (!pointerInsideExpanded(this.expandedEl, event.clientX, event.clientY)) {
        this.hideFramePreview();
      }
    };
    document.addEventListener("pointermove", this._onWindowPointerMove, true);
  },

  updateFramePreview(clientX, clientY, rowEl, debouncedLoad) {
    if (!pointerInsideExpanded(this.expandedEl, clientX, clientY)) {
      this.hideFramePreview();
      return;
    }

    if (!rowEl || !this.expandedEl?.contains(rowEl)) {
      this.hideFramePreview();
      return;
    }
    ...
    const tileDuration = parseFloat(rowEl.dataset.tileDuration) || LARGE_SECS_PER_TILE;
    const rowRect      = rowEl.getBoundingClientRect();
    const ratio        = clamp((clientX - rowRect.left) / rowRect.width, 0, 1);
    ...
    this.framePreviewEl.removeAttribute("hidden");
    this.framePreviewEl.setAttribute("aria-hidden", "false");
    this.attachWindowPointerTracking();
    positionFramePreview(this.framePreviewEl, this.expandedEl, clientX, rowRect);

    debouncedLoad(t);
  },

  refreshFramePreviewAtPointer(debouncedLoad) {
    if (
      this._previewPointerX == null ||
      this._previewPointerY == null ||
      this.framePreviewEl?.classList.contains("hidden")
    ) {
      return;
    }

    if (!pointerInsideExpanded(this.expandedEl, this._previewPointerX, this._previewPointerY)) {
      this.hideFramePreview();
      return;
    }

    const target = document.elementFromPoint(this._previewPointerX, this._previewPointerY);
    const rowEl  = target?.closest?.("div.timeline-large-row");
    this.updateFramePreview(this._previewPointerX, this._previewPointerY, rowEl, debouncedLoad);
  },
  ...
  attachFramePreview() {
    ...
    this.expandedEl.addEventListener("pointermove", (event) => {
      ...
      this._previewPointerX = event.clientX;
      this._previewPointerY = event.clientY;
      this.updateFramePreview(event.clientX, event.clientY, rowEl, debouncedLoad);
    });
    ...
    // Wheel scroll does not emit pointermove — re-anchor preview to row under cursor.
    this._onFramePreviewScroll = () => {
      this.refreshFramePreviewAtPointer(debouncedLoad);
    };
    this.expandedEl.addEventListener("scroll", this._onFramePreviewScroll, { passive: true });
  },
};
